...
and saves them as Markdown and PDF files.
...
- Create approximately 15 sections.
...
STANDARD_DOCUMENT_PROMPT = """
Create the complete internal telecom policy knowledge-base document.
...
- Start with `## Overview`.
- End with `## FAQs`.
- Create a comprehensive operational document.
- Cover scope, eligibility, requirements, procedures, rules, exceptions,
  responsibilities, timelines, escalation, and examples where applicable.
- Include exactly 5 FAQs at the end.
- Use `Q:` and `A:` format for FAQs.
- Do not invent unsupported numbers, prices, thresholds, dates, or rules.
- Do not mention AI or document generation.
- Do not put the document title as the first heading.
""".strip()
...
class DocumentGenerator:
    ...
    def generate(self, metadata: KnowledgeMetadata) -> KnowledgeDocument:
        ...
        Standard documents use one LLM request.
        Long documents use outline + section generation + FAQ generation.
        """
        ...
        if metadata.sections > 0:
            ...
            body = self._generate_standard_document(metadata)
    ...
    def _generate_standard_document(
        self,
        metadata: KnowledgeMetadata,
    ) -> str:
        """Generate a normal document with one LLM request."""

        prompt = STANDARD_DOCUMENT_PROMPT.format(
    ...
    def _generate_long_document(
        ...
        logger.info(
            "Using long-document generation for %s",
            metadata.doc_id,
        )
        ...
        if not sections:
            ...
            return self._generate_standard_document(metadata)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--keep-md",
        action="store_true",
        help="Keep generated Markdown files.",
    )
